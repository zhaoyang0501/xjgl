jQuery.adminScore = {
		...
		initSearchDataTable : function() {
			if (this.scoreDataTable == null) {
				this.scoreDataTable = $('#dt_table_view').dataTable({
					...
					"bProcessing" : true,
					"bSort" : false,
					"sAjaxSource" : $.ace.getContextPath() + "/admin/score/list",
				...
				var oSettings = this.scoreDataTable.fnSettings();
				oSettings._iDisplayStart = 0;
				this.scoreDataTable.fnDraw(oSettings);
			}
		...
		deleteScore :function(id){
			bootbox.confirm( "是否确认删除？", function (result) {
	            if(result){
	            	$.ajax({
	        			type : "get",
	        			url : $.ace.getContextPath() + "/admin/score/delete/"+id,
	        			dataType : "json",
		...
		showEdit: function (id){
			...
			$.ajax({
    			type : "get",
    			url : $.ace.getContextPath() + "/admin/score/get/"+id,
    			dataType : "json",
    			success : function(json) {
    				if(json.state=='success'){
		...
		saveScore: function(id){
			$.ajax({
    			type : "post",
    			url : $.ace.getContextPath() + "/admin/score/save",
    			data:$("form").serialize(),
    			dataType : "json",
    			success : function(json) {
    				if(json.state=='success'){
